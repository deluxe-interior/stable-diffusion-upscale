...
def preprocess(input_frames):
    ...
    for pointer in range(len(input_frames)):
        ...
        #logger.info('Added frame {}'.format(frame))
...
def load_video(vid_path):
    logger.info('Loading video {}'.format(vid_path))
...
def save_video(video, save_dir, file_name, fps=16.0):
    ...
    tmp_path = os.path.join(save_dir, 'tmp.mov')
    cmd = f'ffmpeg -y -f image2 -framerate {fps} -i {temp_dir}/%06d.png -level 3 -c:v ffv1 -context 1 -slices 4 -slicecrc 1 -g 1 -pix_fmt yuva444p16le {tmp_path}'
