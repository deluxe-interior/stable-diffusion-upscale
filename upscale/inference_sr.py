...
from upscale.color_fix import adain_color_fix
...
class STAR():
    ...
    def enhance_a_video(self, video_path, prompt):
        ...
        logger.info('Input frames per second: {}'.format(input_fps))

        video_data = preprocess(input_frames)
        _, _, h, w = video_data.shape
        logger.info('Input resolution: {}'.format((w, h)))
        target_h, target_w = h * self.upscale, w * self.upscale   # adjust_resolution(h, w, up_scale=4)
        logger.info('Target resolution: {}'.format((target_w, target_h)))
        logger.info('Steps {}'.format(self.steps))
        ...
        with torch.no_grad():
            ...
            logger.info('Processing torch model.test')
            output = self.model.test(data_tensor, total_noise_levels, steps=self.steps, solver_mode=self.solver_mode, guide_scale=self.guide_scale, max_chunk_len=self.max_chunk_len)
            logger.info('Finished torch model.test')
